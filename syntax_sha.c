...
// define these values in the macro instead of using it as a variable since it is fixed
#define NUM_OF_T 100
...
//  for (i=0; i<size; i++) paddedmsg[i]=message[i];
  paddedmsg[size]= 0x80;
//  for (i=size+1; i<64; i++) paddedmsg[i]=0x00;
  paddedmsg[63] = bitlen;
...
//  for (i=0; i<64; i++) paddedmsg[i]=0x00;
  paddedmsg[63] = bitlen;
  paddedmsg[62] = bitlen >> 8;
  paddedmsg[61] = bitlen >> 16;
  paddedmsg[60] = bitlen >> 24;
...
void sha1_md(unsigned char message[], int size, unsigned int hash[5]) {
  ...
  register int Q= size/64;
  register int R= size%64;
  unsigned char msg[R];
...
//    for (i=R+1; i<64; i++) msgTBH[i]=0x00;
    sha1_process(hash, msgTBH);
...
void sha256_process(unsigned int hash[], unsigned char msg[]) {
  ...
  unsigned int W[64];
  register int i=0;
  register unsigned int A, B, C, D, E, F, G, H, T1, T2;
  while (i < 16){
      W[i] = (((unsigned) msg[i * 4]) << 24) |
             (((unsigned) msg[i * 4 + 1]) << 16) |
             (((unsigned) msg[i * 4 + 2]) << 8) |
...
//  for (i=size+1; i<128; i++) paddedmsg[i]=0x00;
...
//  for (i=0; i<128; i++) paddedmsg[i]=0x00;
  paddedmsg[127] = bitlen;
  paddedmsg[126] = bitlen >> 8;
  paddedmsg[125] = bitlen >> 16;
  paddedmsg[124] = bitlen >> 24;
...
//  for(i = 16; i < 80; i++) W[i] = sigma5121(W[i-2])+W[i-7]+sigma5120(W[i-15])+ W[i-16];
  A = hash[0];
  B = hash[1];
  C = hash[2];
  D = hash[3];
...
//      for (i=0;i<NUM_OF_T;i++) sha1_md(msg4, TEST_SIZE, hash1);
      finish = clock();
  ...
  if (shatype==2) {
    sha256_md(msg4, TEST_SIZE,hash2);
...
//      for (i=0;i<NUM_OF_T;i++) sha256_md(msg4, TEST_SIZE,hash2);
      finish = clock();
      seconds = ((double)(finish - start))/CLOCKS_PER_SEC;
  ...
  if (shatype==3) {
    sha512_md(msg4, TEST_SIZE,hash3);
...
//      for (i=0;i<NUM_OF_T;i++) sha512_md(msg4, TEST_SIZE,hash3);
      finish = clock();
      seconds = ((double)(finish - start))/CLOCKS_PER_SEC;
